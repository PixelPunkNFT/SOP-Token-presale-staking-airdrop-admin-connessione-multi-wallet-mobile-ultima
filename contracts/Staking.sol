...
contract Staking is ReentrancyGuard, Ownable {
    ...
    uint256 public maxStakingCap = 5_000_000 * 1e18; // 5 million tokens initially
    ...
    function stake(uint256 _amount) external nonReentrant {
        ...
        require(totalStaked + _amount <= maxStakingCap, "Would exceed max staking cap");
    ...
    function setMaxStakingCap(uint256 _newCap) external onlyOwner {
        require(_newCap >= totalStaked, "New cap must be >= current total staked");
        maxStakingCap = _newCap;
    }
    ...
    function getStakeInfo(address _staker) external view returns (
        ...
        StakeInfo memory stakeData = stakes[_staker];
        return (
            stakeData.amount,
            stakeData.startTime,
            calculateRewards(_staker) + stakeData.unclaimableRewards
        );
    }
    ...
    function burn(uint256 amount) external onlyOwner {
        ...
        for(uint256 i = 0; i < stakersArray.length; i++) {
            if(stakes[stakersArray[i]].amount > 0) {
                reward = calculateRewards(stakersArray[i]);
                totalPendingRewards += reward + stakes[stakersArray[i]].unclaimableRewards;
            }
        }
