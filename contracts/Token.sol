...
/**
 *  SOP - SMART OPERATIONS PLATFORM
 * @dev Implementation of the SOP Token with advanced features including staking, 
 * airdrop, presale, and liquidity management capabilities.
 * 
 * This token is part of the White Paper ecosystem.
 * For more information, visit: https://github.com/PixelPunkNFT
 */
...
contract Token is ERC20, Ownable {
    ...
    function _transfer(
        ...
        bool isSpecialAddress = recipient == presaleContract || 
    ...
    function getLockedTokens(address account) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < userLocks[account].length; i++) {
            LockInfo memory lock = userLocks[account][i];
            if (lock.amount == 0 || currentTime >= lock.lockEndTime) {
                continue;
            }
            
            if (currentTime >= lock.partialUnlockTime) {
