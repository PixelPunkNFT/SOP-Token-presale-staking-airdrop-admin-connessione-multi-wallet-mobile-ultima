...
contract Presale is Ownable, ReentrancyGuard {
    ...
    uint256 public lastPriceCalculationTime;
    ...
    function getCurrentPrice() public view returns (uint256) {
        if (block.timestamp < lastPriceCalculationTime + PRICE_INCREASE_INTERVAL) {
    ...
    function updateLastCalculatedPrice() internal {
        lastCalculatedPrice = getCurrentPrice();
        lastPriceCalculationTime = block.timestamp;
    }
    ...
    function setMaxContribution(uint256 _newMax) external onlyOwner {
        require(_newMax > 0, "Max contribution must be greater than 0");
        maxContribution = _newMax;
        emit MaxContributionUpdated(_newMax);
    }

    function withdrawAccumulatedEth(uint256 _amount) external onlyOwner {
    ...
    function claimRefund() external nonReentrant {
        ...
        require(block.timestamp > presaleStartTime + 120 days, "Refund not available yet");
    ...
    function participate() external payable nonReentrant whenNotStopped {
        require(!presaleFinalized, "Presale finalized");
        require(address(token) != address(0), "Token not set");
        require(msg.value + contributions[msg.sender] <= maxContribution, "Exceeds max contribution");
